...
Step 1 is usually where the headache starts, and is commonly done using Babel and Webpack. However, we can use [ESBuild](https://esbuild.github.io/) instead, which can do both transpiling _and_ bundling, and is also incredibly fast as it's written in Rust and very easy to use.
